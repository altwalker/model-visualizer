...
function validataModel(model) {
  ...
  if (!isNameValid(model.name)) { throw new ValidationError(`Invalid model name: ${model.name}.`) }
...
function validateVertex(vertex) {
  ...
  if (!isNameValid(vertex.name)) { throw new ValidationError(`Invalid vertex name: ${vertex.name}.`) }
}
...
function validateEdge(edge) {
  ...
  if (!isNameValid(edge.name)) { throw new ValidationError(`Invalid edge name: ${edge.name}.`) }
...
export function isNameValid(name) {
  ...
  if (!isIdentifier.test(name)) { return false }
...
export function isWeightValid(weight) {
  ...
  if (!weight || weight < 0 || weight > 1) { return false }
  return true
}
